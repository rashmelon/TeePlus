	...
	<div class="show-single-category-compinations">
		<vx-card class="my-5" collapse-action title="Combinations">
			...
			<div class="text-center mt-4">
				<vs-button  color="primary" type="filled" @click="updateCombination">Edit compination</vs-button>
			</div>
		</vx-card>
	</div>
</template>
  ...
  export default {
    ...
    methods: {
      getProductCombinations(){
        ...
        this.$store.dispatch('combination/getData', `?product=${this.prodId}`)
          .then(response => {
            this.productCombinations = response.data.data;
            console.log('product Combinations: ', this.productCombinations)
          })
          .catch(error => {
            console.log(error);
            this.$vs.notify({
      ...
      updateCombination(item) {
        this.$vs.notify({title: 'Error', text: 'Not handeled yet', iconPack: 'feather', icon: 'icon-alert-circle', color: 'danger'});
