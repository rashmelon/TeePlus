  ...
  methods: {
    create: function create() {
      ...
      this.$validator.validateAll().then(function (result) {
        if (result) {
          ...
          console.log(_this.tempProducts);

          for (var i = 0; i < _this.tempProducts.length; i++) {
            var item = {};
            item.quantity = _this.tempProducts[i].quantity;
            item.product_id = _this.tempProducts[i].product.id;
            item.price_combination_id = _this.tempProducts[i].priceCombination.id;
            item.design_id = _this.tempProducts[i].design.id;

            _this.order.orderProducts.push(item);
          }

          var sentObject = _objectSpread({}, _this.order); // create new object for sending object without extra data
          ...
          _this.$store.dispatch('order/create', form_data).then(function (response) {
            _this.$vs.notify({
              title: 'Success',
    ...
    getShippingPrice: function getShippingPrice() {
      ...
      this.$store.dispatch('shippingPrice/getData', this.payload).then(function (response) {
        _this2.shippingPrices = response.data.data;
      }).catch(function (error) {
        console.log(error);
        ...
        _this2.$vs.loading.close(_this2.$refs.create.$el);
      });
    },
    ...
    getCombinations: function getCombinations() {
      ...
      this.$store.dispatch('combination/getData', "?category=".concat(this.cartItem.category.id)).then(function (response) {
        _this5.combinations = response.data.data;
        console.log(_this5.combinations);
      }).catch(function (error) {
        console.log(error);
    ...
    getStatuses: function getStatuses() {
      ...
      this.$store.dispatch('status/getData', this.payload).then(function (response) {
        ...
        _this6.$vs.loading.close(_this6.$refs.create.$el);
      });
    },
                ...
                _c("div", { staticClass: "vx-row" }, [
                  ...
                  _c(
                    ...
                    [
                      _c("vs-input", {
                        staticClass: "w-full",
                        attrs: { placeholder: "Additional number" },
                        model: {
                  ...
                  _c(
                    ...
                    [
                      _c("vs-input", {
                        staticClass: "w-full",
                        attrs: { placeholder: "Shipping notes" },
                        model: {
                  ...
                  _c(
                    ...
                    [
                      _c("vs-input", {
                        staticClass: "w-full",
                        attrs: { placeholder: "Additional fees details" },
                        model: {
