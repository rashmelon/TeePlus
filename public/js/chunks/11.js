  ...
  methods: {
    ...
    getCategories: function getCategories() {
      ...
      this.$store.dispatch('category/getData', this.payload).then(function (response) {
        _this3.categories = response.data.data; // get priceCombinations of every category seperately

        _this3.categories.forEach(function (cat) {
          _this3.getCombinations(cat.id);
        });
    ...
    getCombinations: function getCombinations(catId) {
      ...
      this.$store.dispatch('combination/getData', "?category=".concat(catId)).then(function (response) {
        ...
        for (var index = 0; index < _this4.categories.length; index++) {
          if (_this4.categories[index].id == catId) {
            _this4.categories[index].priceCombinations = response.data.data;
          }
        }
      }).catch(function (error) {
        console.log(error);
                          ...
                          _c(
                            ...
                            [
                              _c(
                                ...
                                _vm._l(
                                  _vm.form.category.priceCombinations,
                                  function(item, index) {
                                    return _c("vs-select-item", {
                                      key: index,
                                      attrs: {
